...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  /* ── Import Results Tab ── */
  .import-tab {{ color: #7c3aed !important; }}
  .view-tab.import-tab.active {{ color: #6d28d9 !important; border-bottom-color: #7c3aed !important; }}
  .import-wrapper {{ display: flex; flex: 1; min-height: 0; overflow: hidden; }}
  .import-sidebar {{ width: 260px; flex-shrink: 0; background: var(--surface); border-right: 2px solid var(--border); display: flex; flex-direction: column; overflow: hidden; }}
  .import-sidebar-header {{ padding: 12px 14px; font-size: 13px; font-weight: 700; color: #6d28d9; background: #f5f3ff; border-bottom: 1px solid var(--border); }}
  .import-drop-zone {{ margin: 12px; border: 2px dashed #a78bfa; border-radius: 10px; padding: 24px 12px; text-align: center; cursor: pointer; transition: all 0.2s; background: #faf5ff; }}
  .import-drop-zone:hover, .import-drop-zone.drag-over {{ border-color: #7c3aed; background: #ede9fe; }}
  .import-drop-icon {{ font-size: 28px; margin-bottom: 6px; }}
  .import-drop-text {{ font-size: 12px; color: #6d28d9; line-height: 1.5; }}
  .import-queue {{ flex: 1; overflow-y: auto; padding: 4px 8px; }}
  .import-queue-item {{ display: flex; align-items: center; gap: 8px; padding: 7px 8px; margin-bottom: 4px; background: white; border: 1px solid var(--border); border-radius: 6px; font-size: 11px; }}
  .import-queue-item.processing {{ border-color: #a78bfa; background: #faf5ff; }}
  .import-queue-item.done {{ border-color: #86efac; background: #f0fdf4; }}
  .import-queue-item.error {{ border-color: #fca5a5; background: #fef2f2; }}
  .import-queue-thumb {{ width: 36px; height: 36px; object-fit: cover; border-radius: 4px; flex-shrink: 0; }}
  .import-queue-info {{ flex: 1; min-width: 0; }}
  .import-queue-name {{ white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 500; color: var(--text); }}
  .import-queue-state {{ font-size: 10px; color: var(--text-muted); margin-top: 1px; }}
  .import-queue-remove {{ flex-shrink: 0; width: 18px; height: 18px; border-radius: 50%; background: #e5e7eb; border: none; cursor: pointer; font-size: 10px; display: flex; align-items: center; justify-content: center; color: #6b7280; }}
  .import-queue-remove:hover {{ background: #fca5a5; color: #9c0006; }}
  .import-sidebar-footer {{ padding: 10px 12px; border-top: 1px solid var(--border); display: flex; gap: 8px; }}
  .import-btn-primary {{ flex: 1; padding: 7px 12px; background: #7c3aed; color: white; border: none; border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; transition: background 0.15s; }}
  .import-btn-primary:hover:not(:disabled) {{ background: #6d28d9; }}
  .import-btn-primary:disabled {{ opacity: 0.4; cursor: not-allowed; }}
  .import-btn-secondary {{ padding: 7px 12px; background: white; color: var(--text-muted); border: 1px solid var(--border); border-radius: 6px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.15s; }}
  .import-btn-secondary:hover {{ background: var(--bg); border-color: #a78bfa; color: #7c3aed; }}
  .import-main {{ flex: 1; display: flex; flex-direction: column; overflow: hidden; }}
  .import-toolbar {{ display: flex; align-items: center; gap: 10px; padding: 8px 16px; background: #f5f3ff; border-bottom: 1px solid var(--border); flex-shrink: 0; }}
  .import-results-area {{ flex: 1; overflow: auto; background: var(--bg); padding: 16px; }}
  .import-empty-state {{ display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; text-align: center; }}
  .import-result-card {{ background: white; border: 1px solid var(--border); border-radius: 10px; margin-bottom: 14px; overflow: hidden; box-shadow: 0 1px 4px rgba(0,0,0,0.06); }}
  .import-result-header {{ display: flex; align-items: center; gap: 10px; padding: 10px 14px; background: #f5f3ff; border-bottom: 1px solid #e9d5ff; }}
  .import-result-thumb {{ width: 48px; height: 36px; object-fit: cover; border-radius: 4px; flex-shrink: 0; }}
  .import-result-filename {{ font-size: 11px; color: #6d28d9; font-weight: 600; flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
  .import-result-badge {{ font-size: 10px; background: #7c3aed; color: white; padding: 2px 8px; border-radius: 10px; white-space: nowrap; }}
  .import-field-grid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px; padding: 12px 14px; }}
  .import-field {{ display: flex; flex-direction: column; gap: 3px; }}
  .import-field label {{ font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); }}
  .import-field input, .import-field select {{ padding: 5px 8px; border: 1px solid var(--border); border-radius: 5px; font-size: 12px; font-family: inherit; background: white; color: var(--text); transition: border-color 0.15s; }}
  .import-field input:focus, .import-field select:focus {{ outline: none; border-color: #a78bfa; }}
  .import-field-wide {{ grid-column: 1 / -1; }}
  .import-result-actions {{ display: flex; justify-content: flex-end; gap: 8px; padding: 8px 14px; border-top: 1px solid #f0e6ff; background: #fdfcff; }}
  .import-result-discard {{ padding: 5px 12px; background: white; color: #dc2626; border: 1px solid #fca5a5; border-radius: 5px; font-size: 11px; cursor: pointer; }}
  .import-result-discard:hover {{ background: #fef2f2; }}
  .import-spinner {{ display: inline-block; width: 12px; height: 12px; border: 2px solid #e9d5ff; border-top-color: #7c3aed; border-radius: 50%; animation: spin 0.6s linear infinite; }}
  ...
  .surface-btn[data-val="Acrobat Reader"].active {{ color: #1d4ed8; }}
  .surface-btn[data-val="Acrobat Reduced Mode"].active {{ color: #7c3aed; }}
...
<div class="header">
  ...
  <div class="controls">
    ...
    <div class="surface-toggle" id="surfaceToggle">
      ...
      <button class="surface-btn" data-val="Acrobat Reader">Reader</button>
      <button class="surface-btn" data-val="Acrobat Reduced Mode">Reduced Mode</button>
    </div>
...
<div class="view-tabs">
  ...
  <div class="view-tab import-tab" data-view="import">⬆ Import Results</div>
</div>
...
<div class="view-panel" id="importView">
  <div class="import-wrapper">
    <div class="import-sidebar">
      <div class="import-sidebar-header">📸 Screenshot Queue</div>
      <div class="import-drop-zone" id="importDropZone">
        <div class="import-drop-icon">⬆</div>
        <div class="import-drop-text">Drop screenshots here<br>or click to browse</div>
        <input type="file" id="importFileInput" accept="image/*" multiple style="display:none;">
      </div>
      <div class="import-queue" id="importQueue"></div>
      <div class="import-sidebar-footer">
        <button class="import-btn-primary" id="importRunBtn" onclick="importRunAll()" disabled>Extract All</button>
        <button class="import-btn-secondary" onclick="importClearAll()">Clear</button>
      </div>
    </div>
    <div class="import-main">
      <div class="import-toolbar">
        <span style="font-weight:700;color:#6d28d9;font-size:13px;">Extracted Results</span>
        <span id="importStatus" style="font-size:11px;color:var(--text-muted);margin-left:8px;"></span>
        <span style="flex:1;"></span>
        <button class="import-btn-secondary" onclick="importCopyExcel()" id="importCopyBtn" style="display:none;">📋 Copy for Excel</button>
        <button class="import-btn-primary" onclick="importDownloadCSV()" id="importCSVBtn" style="display:none;">⬇ Download CSV</button>
        <button onclick="importClearApiKey()" style="padding:4px 8px;font-size:10px;color:var(--text-muted);border:none;background:none;cursor:pointer;" title="Clear saved Anthropic API key">⚙ API key</button>
      </div>
      <div class="import-results-area" id="importResultsArea">
        <div class="import-empty-state" id="importEmptyState">
          <div style="font-size:48px;margin-bottom:16px;">📊</div>
          <div style="font-size:15px;font-weight:600;color:var(--text-muted);margin-bottom:8px;">No results yet</div>
          <div style="font-size:12px;color:var(--text-light);line-height:1.6;">Drop experiment result screenshots on the left.<br>Claude vision will extract test names, dates, metrics, and winners<br>into rows ready to paste into the Results sheet.</div>
        </div>
      </div>
    </div>
  </div>
...
function importInit() {{
  ...
  if (!dz || dz._importInited) return;
  dz._importInited = true;
  dz.addEventListener('click', () => fi.click());
  fi.addEventListener('change', e => {{ importAddFiles(Array.from(e.target.files)); fi.value = ''; }});
  dz.addEventListener('dragover', e => {{ e.preventDefault(); dz.classList.add('drag-over'); }});
  dz.addEventListener('dragleave', () => dz.classList.remove('drag-over'));
  dz.addEventListener('drop', e => {{
    e.preventDefault(); dz.classList.remove('drag-over');
    importAddFiles(Array.from(e.dataTransfer.files).filter(f => f.type.startsWith('image/')));
  }});
}}

function importAddFiles(files) {{
  files.forEach(f => {{
    const id = Date.now() + Math.random();
    const reader = new FileReader();
    reader.onload = ev => {{
      importQueue.push({{ id, file: f, dataUrl: ev.target.result, status: 'pending', result: null }});
      importRenderQueue();
    }};
...
function importRenderQueue() {{
  const el = document.getElementById('importQueue');
  el.innerHTML = importQueue.map(item => {{
    const stateIcon = item.status === 'pending' ? '⏳' : item.status === 'processing' ? '<span class="import-spinner"></span>' : item.status === 'done' ? '✅' : '❌';
    const stateText = item.status === 'pending' ? 'Ready' : item.status === 'processing' ? 'Extracting…' : item.status === 'done' ? 'Extracted' : 'Error';
    return `<div class="import-queue-item ${{item.status}}" data-id="${{item.id}}">
      <img class="import-queue-thumb" src="${{item.dataUrl}}">
      <div class="import-queue-info">
        <div class="import-queue-name">${{item.file.name}}</div>
        <div class="import-queue-state">${{stateIcon}} ${{stateText}}</div>
      </div>
      <button class="import-queue-remove" onclick="importRemoveItem(${{item.id}})" title="Remove">✕</button>
    </div>`;
  }}).join('');
  const btn = document.getElementById('importRunBtn');
  btn.disabled = importQueue.filter(i => i.status === 'pending').length === 0;
}}

function importRemoveItem(id) {{
  importQueue = importQueue.filter(i => i.id !== id);
  importRenderQueue();
}}

function importClearAll() {{
  importQueue = [];
  importResults = [];
  importRenderQueue();
  importRenderResults();
...
async function importRunAll() {{
  const pending = importQueue.filter(i => i.status === 'pending');
  if (!pending.length) return;
  document.getElementById('importStatus').textContent = `Extracting ${{pending.length}} screenshot${{pending.length > 1 ? 's' : ''}}…`;
  for (const item of pending) {{
    item.status = 'processing';
    importRenderQueue();
    try {{
      const extracted = await importExtract(item);
      item.status = 'done';
      item.result = extracted;
      importResults.push(...extracted.map((r, i) => ({{ ...r, _id: String(item.id) + '_' + i, _thumb: item.dataUrl, _filename: item.file.name }})));
    }} catch(e) {{
      item.status = 'error';
      console.error('Extract error:', e);
      if (e.message && e.message.includes('401')) importClearApiKey();
    }}
    importRenderQueue();
    importRenderResults();
  }}
  document.getElementById('importStatus').textContent = `Done — ${{importResults.length}} row${{importResults.length !== 1 ? 's' : ''}} extracted`;
}}

function importGetApiKey() {{ return localStorage.getItem('anthropic_import_key') || ''; }}
function importSetApiKey(k) {{ localStorage.setItem('anthropic_import_key', k); }}
function importClearApiKey() {{ localStorage.removeItem('anthropic_import_key'); document.getElementById('importStatus').textContent = 'API key cleared.'; }}
function importPromptApiKey() {{
  const k = prompt('Enter your Anthropic API key to enable screenshot extraction.\n\nGet one at https://console.anthropic.com/settings/keys\n\nIt will be saved in your browser localStorage for this tool only.');
  if (k && k.trim()) {{ importSetApiKey(k.trim()); return k.trim(); }}
  return null;
}}

async function importExtract(item) {{
  let apiKey = importGetApiKey();
  if (!apiKey) {{
    apiKey = importPromptApiKey();
    if (!apiKey) throw new Error('No API key');
  }}
  const base64 = item.dataUrl.split(',')[1];
  const mediaType = item.file.type || 'image/png';
  const prompt = `You are analyzing a screenshot of an A/B experiment results dashboard (Adobe Acrobat Reader or Reduced Mode product testing).

Extract ALL experiment results visible in the screenshot. For each experiment return a JSON array of objects with these exact keys:
- "product": "Acrobat Reader" or "Acrobat Reduced Mode" (infer from context)
- "test_name": full test name including RGS ID (e.g. "RGS0480 - Export PDF Direct Upsell Modal - Reader - Multi - WW - EN")
- "start_date": ISO date string YYYY-MM-DD or ""
- "end_date": ISO date string YYYY-MM-DD or "" (empty if still running)
- "qgnarr": numeric QGNARR value or null
- "gnarr_lift": decimal lift e.g. 0.022 for 2.2%, or null
- "ctr": decimal CTR lift or null
- "units_pct": decimal units % lift or null
- "winner": "Challenger 1", "Challenger 2", "Control", "Rollout", or null
- "details": any decision or notable detail text, or ""
- "gds_link": URL if visible, or ""
...
function importRenderResults() {{
  const area = document.getElementById('importResultsArea');
  if (!importResults.length) {{
    area.innerHTML = '<div class="import-empty-state"><div style="font-size:48px;margin-bottom:16px;">📊</div><div style="font-size:15px;font-weight:600;color:var(--text-muted);margin-bottom:8px;">No results yet</div><div style="font-size:12px;color:var(--text-light);line-height:1.6;">Drop experiment result screenshots on the left.<br>Claude vision will extract test names, dates, metrics, and winners<br>into rows ready to paste into the Results sheet.</div></div>';
    document.getElementById('importCopyBtn').style.display = 'none';
    document.getElementById('importCSVBtn').style.display = 'none';
    return;
  }}
  document.getElementById('importCopyBtn').style.display = '';
  document.getElementById('importCSVBtn').style.display = '';
  area.innerHTML = importResults.map(r => `
    <div class="import-result-card" data-rid="${{r._id}}">
      <div class="import-result-header">
        <img class="import-result-thumb" src="${{r._thumb}}">
        <span class="import-result-filename">${{r._filename}}</span>
        <span class="import-result-badge">extracted</span>
      </div>
      <div class="import-field-grid">
        <div class="import-field import-field-wide">
          <label>Test Name</label>
          <input type="text" value="${{esc(r.test_name || '')}}" data-field="test_name" data-rid="${{r._id}}" style="width:100%;">
        </div>
        <div class="import-field">
          <label>Product</label>
          <select data-field="product" data-rid="${{r._id}}">
            <option value="Acrobat Reader" ${{r.product === 'Acrobat Reader' ? 'selected' : ''}}>Acrobat Reader</option>
            <option value="Acrobat Reduced Mode" ${{r.product === 'Acrobat Reduced Mode' ? 'selected' : ''}}>Acrobat Reduced Mode</option>
          </select>
        </div>
        <div class="import-field">
          <label>Start Date</label>
          <input type="text" placeholder="YYYY-MM-DD" value="${{esc(r.start_date || '')}}" data-field="start_date" data-rid="${{r._id}}">
        </div>
        <div class="import-field">
          <label>End Date</label>
          <input type="text" placeholder="YYYY-MM-DD or blank" value="${{esc(r.end_date || '')}}" data-field="end_date" data-rid="${{r._id}}">
        </div>
        <div class="import-field">
          <label>Winner</label>
          <select data-field="winner" data-rid="${{r._id}}">
            <option value="">—</option>
            <option value="Challenger 1" ${{r.winner === 'Challenger 1' ? 'selected' : ''}}>Challenger 1</option>
            <option value="Challenger 2" ${{r.winner === 'Challenger 2' ? 'selected' : ''}}>Challenger 2</option>
            <option value="Control" ${{r.winner === 'Control' ? 'selected' : ''}}>Control</option>
            <option value="Rollout" ${{r.winner === 'Rollout' ? 'selected' : ''}}>Rollout</option>
          </select>
        </div>
        <div class="import-field">
          <label>QGNARR</label>
          <input type="text" value="${{r.qgnarr != null ? r.qgnarr : ''}}" data-field="qgnarr" data-rid="${{r._id}}">
        </div>
        <div class="import-field">
          <label>GNARR Lift</label>
          <input type="text" value="${{r.gnarr_lift != null ? r.gnarr_lift : ''}}" data-field="gnarr_lift" data-rid="${{r._id}}">
        </div>
        <div class="import-field">
          <label>CTR</label>
          <input type="text" value="${{r.ctr != null ? r.ctr : ''}}" data-field="ctr" data-rid="${{r._id}}">
        </div>
        <div class="import-field">
          <label>Units %</label>
          <input type="text" value="${{r.units_pct != null ? r.units_pct : ''}}" data-field="units_pct" data-rid="${{r._id}}">
        </div>
        <div class="import-field import-field-wide">
          <label>Decision / Details</label>
          <input type="text" value="${{esc(r.details || '')}}" data-field="details" data-rid="${{r._id}}" style="width:100%;">
        </div>
        <div class="import-field import-field-wide">
          <label>GDS Link</label>
          <input type="text" value="${{esc(r.gds_link || '')}}" data-field="gds_link" data-rid="${{r._id}}" style="width:100%;">
        </div>
      </div>
      <div class="import-result-actions">
        <button class="import-result-discard" onclick="importDiscardRow('${{r._id}}')">✕ Discard Row</button>
      </div>
    </div>`).join('');
  area.querySelectorAll('[data-field]').forEach(el => {{
    el.addEventListener('change', e => {{
      const row = importResults.find(r => r._id === e.target.dataset.rid);
...
function importDiscardRow(rid) {{
  importResults = importResults.filter(r => r._id !== rid);
  importRenderResults();
  document.getElementById('importStatus').textContent = `${{importResults.length}} row${{importResults.length !== 1 ? 's' : ''}} remaining`;
}}

function importDownloadCSV() {{
  const headers = ['Product','Test Name','Start Date','End Date','QGNARR','GNARR Lift','CTR','Units %','Winner','GDS Link','Decision/Details'];
  const lines = [headers.join(','), ...importResults.map(r =>
    [r.product,r.test_name,r.start_date,r.end_date,r.qgnarr!=null?r.qgnarr:'',r.gnarr_lift!=null?r.gnarr_lift:'',r.ctr!=null?r.ctr:'',r.units_pct!=null?r.units_pct:'',r.winner,r.gds_link,r.details]
    .map(v => '"' + String(v||'').replace(/"/g,'""') + '"').join(','))];
  const a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([lines.join('\n')], {{type:'text/csv'}}));
  a.download = 'import_results.csv'; a.click();
}}

function importCopyExcel() {{
  const lines = importResults.map(r =>
    [r.product,r.test_name,r.start_date,r.end_date,r.qgnarr!=null?r.qgnarr:'',r.gnarr_lift!=null?r.gnarr_lift:'',r.ctr!=null?r.ctr:'',r.units_pct!=null?r.units_pct:'',r.winner,r.gds_link,r.details].join('\t'));
  navigator.clipboard.writeText(lines.join('\n')).then(() => {{
    const btn = document.getElementById('importCopyBtn');
...
document.querySelectorAll('.view-tab').forEach(tab => {{
  if (tab.dataset.view === 'import') tab.addEventListener('click', importInit);
}});
importInit();
</script>
</body>
